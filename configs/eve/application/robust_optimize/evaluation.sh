...
# Use the robust_optimize venv Python which has all required dependencies
# (torch, neuralop, etc.) installed.
EVAL_PYTHON="/wuyangxin/robust_optimize/.venv/bin/python3"
if [[ ! -x "$EVAL_PYTHON" ]]; then
  echo "ERROR: Evaluation Python not found at $EVAL_PYTHON" >&2
  exit 1
fi
...
try:
    ...
    print("evaluate.py timed out after 3600 seconds", file=sys.stderr)
...
"$EVAL_PYTHON" - "$tmp_stdout" "$score_path" "$error_path" <<'PY'
import json
import sys
...
# Read the evaluator's raw stdout. The evaluate.py may print training logs
# to stdout before the final JSON line, so we extract the last JSON object.
